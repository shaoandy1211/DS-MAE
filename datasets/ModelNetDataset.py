...
class ModelNet40_SVM(Dataset):
    ...
    def __init__(self, config): 
        self.data, self.label = load_modelnet_data(config, config.subset)
        self.npoints = config.npoints
        self.partition = config.subset      
        print_log(f'[DATASET] sample out {self.npoints} points', logger = 'ModelNet40_SVM')  
        print_log(f'[DATASET] {self.data.shape[0]} instances were loaded', logger = 'ModelNet40_SVM')
